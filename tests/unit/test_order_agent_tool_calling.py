"""
Unit tests for OrderAgentToolCalling — the chained tool-calling
...
def _stub_turn_context(
    ...
    Use as ``patch("app.agents.order_agent_tool_calling.build_turn_context",
...
class TestImpossibleEnvelopeGuard:
    ...
    def test_ready_to_confirm_with_empty_cart_downgrades_to_chat(self):
        from app.agents.order_agent_tool_calling import _guard_impossible_envelope
...
class TestProductNotFoundRecovery:
    def test_not_found_becomes_structured_data_not_exception(self):
        ...
        with patch.object(OrderAgentToolCalling, "llm", llm), \
             patch("app.agents.order_agent_tool_calling.order_tools", [add_to_cart_mock]), \
             patch("app.agents.order_agent_tool_calling.conversation_service"), \
             patch("app.agents.order_agent_tool_calling.tracer"), \
             patch(
                 "app.agents.order_agent_tool_calling.render_response",
...
class TestToolErrorRecovery:
    def test_tool_exception_surfaced_to_model(self):
        ...
        with patch.object(OrderAgentToolCalling, "llm", llm), \
             patch("app.agents.order_agent_tool_calling.order_tools", [place_order_mock]), \
             patch("app.agents.order_agent_tool_calling.conversation_service"), \
             patch("app.agents.order_agent_tool_calling.tracer"), \
             patch(
                 "app.agents.order_agent_tool_calling.render_response",
...
class TestMaxIterationsAndFallback:
    def test_runaway_tool_loop_synthesizes_chat_envelope(self):
        ...
        with patch.object(OrderAgentToolCalling, "llm", llm), \
             patch("app.agents.order_agent_tool_calling.order_tools", [view_cart_mock]), \
             patch("app.agents.order_agent_tool_calling.conversation_service"), \
             patch("app.agents.order_agent_tool_calling.tracer"), \
             patch(
                 "app.agents.order_agent_tool_calling.render_response",
...
# ---------------------------------------------------------------------------
# Feature flag delegation from legacy OrderAgent
# ---------------------------------------------------------------------------

class TestFeatureFlagDelegation:
    def test_legacy_mode_does_not_invoke_tool_calling_agent(self):
        """Default (no settings.order_agent_mode) → legacy planner."""
        from app.agents.order_agent import OrderAgent
        agent = OrderAgent()
        with patch.object(OrderAgent, "_tool_calling_agent") as v2_factory:
            ctx = {"business_id": "biela", "business": {"settings": {}}}
            try:
                agent.execute(
                    message_body="x", wa_id="x", name="X",
    ...
    def test_tool_calling_mode_delegates(self):
        """settings.order_agent_mode='tool_calling' → v2 agent runs."""
        from app.agents.order_agent import OrderAgent
        agent = OrderAgent()
        ctx = {
            "business_id": "biela",
            "business": {"settings": {"order_agent_mode": "tool_calling"}},
        }
        v2 = MagicMock()
        v2.execute.return_value = {
            "agent_type": "order", "message": "v2 ran", "state_update": {},
        }
        with patch.object(OrderAgent, "_tool_calling_agent", return_value=v2):
            output = agent.execute(
                message_body="hola", wa_id="x", name="X",
                business_context=ctx, conversation_history=[],
            )
        assert output["message"] == "v2 ran"
        v2.execute.assert_called_once()
...
class TestOrderStateAutoDerivation:
    ...
    def test_save_cart_respects_explicit_legacy_state(self):
        """Legacy executor passes state explicitly (e.g. COLLECTING_DELIVERY
        after PROCEED_TO_CHECKOUT). _save_cart must NOT override it
        with a derived value — those legacy transitions can't be
        inferred from contents alone."""
        from app.services import order_tools
        ...
        assert saved["update"]["order_context"]["state"] == "COLLECTING_DELIVERY"
...
class TestPlaceOrderConfirmationGuard:
    ...
    def test_legacy_bypass_skips_guard(self):
        """The legacy executor passes legacy_bypass=True so the new
        tool-level guard doesn't double-gate it. (Legacy enforces its
        own state machine.)"""
        from app.services import order_tools
        ...
        with patch.object(
            ...
            order_tools.session_state_service, "save"
        ), patch.object(
            order_tools, "_get_delivery_fee", return_value=7000,
        ):
            result = order_tools.place_order.invoke({
                "injected_business_context": {
                    "business_id": "biz1",
        ...
        assert "✅" in result or "confirmado" in result.lower()
        fake_create_order.assert_called_once()
...
class TestAwaitingConfirmationHint:
    def test_hint_injected_when_flag_is_set(self):
        ...
        with patch.object(OrderAgentToolCalling, "llm", llm), \
             ...
             patch("app.agents.order_agent_tool_calling.set_awaiting_confirmation"):
            agent.execute(
...
class TestOutOfZonePromptSurface:
    """``_format_business_info_for_prompt`` must surface the
    ...
    def test_out_of_zone_contacts_render_in_prompt(self):
        ...
        rendered = _format_business_info_for_prompt(ctx)
