...
This split exists because pattern-1 (single LLM writes prose AND calls
tools) hallucinated subtotals in production — the model paraphrased
numbers instead of echoing them. With the renderer constrained to
"facts in envelope only", numeric/name hallucination becomes
structurally bounded.

Routed behind ``business.settings.order_agent_mode == "tool_calling"``;
default is the legacy ``OrderAgent``.
...
- One ``order_agent_tool_calling`` LLM span per loop iteration.
...
class OrderAgentToolCalling(BaseAgent):
    """
    ...
    def __init__(self) -> None:
        ...
        logging.info("[ORDER_AGENT_V2] Initialized (chained tool-calling, LLM lazy)")
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        logging.warning(
            "[ORDER_AGENT_V2] turn_in wa_id=%s cart_has_items=%s "
        ...
        try:
            ...
            for iteration in range(MAX_ITERATIONS):
                response = self.llm.invoke(
                    messages,
                    config={
                        "run_name": "order_agent_tool_calling",
                        "metadata": {
                            "wa_id": wa_id,
                ...
                if not tool_calls:
                    ...
                    logging.warning(
                        "[ORDER_AGENT_V2] model returned no tool calls — "
                    ...
                    if tool_fn is None:
                        ...
                        try:
                            ...
                            if tool_name in (
                                ...
                                logging.warning(
                                    "[ORDER_AGENT_V2_DIAG] tool=%s args=%s "
                            ...
                            logging.warning(
                                "[ORDER_AGENT_V2] tool=%s NOT_FOUND query=%r",
                                tool_name, query,
                            )
                            result_str = (
                            ...
                            logging.exception(
                                "[ORDER_AGENT_V2] tool=%s raised: %s",
                                tool_name, exc,
                            )
                            result_str = (
                ...
                logging.warning(
                    "[ORDER_AGENT_V2] max iterations reached without respond() "
                    "(executed=%s)",
                    executed_tools,
                )
            ...
            if handoff_payload is not None:
                ...
                logging.warning(
                    "[ORDER_AGENT_V2] out_of_zone handoff → CS "
                    "city=%s phone=%s",
            ...
            logging.warning(
                "[ORDER_TURN_V2] wa_id=%s turn_id=%s tools=%s iterations=%d "
            ...
            if rendered.get("type") == "cta":
                ...
                logging.warning(
                    "[ORDER_AGENT_V2] CTA send failed — falling back to text body"
                )
            ...
            logging.exception("[ORDER_AGENT_V2] error: %s", exc)
            tracer.end_run(
    ...
    def _persist(self, wa_id: str, business_id: str, text: str) -> None:
        ...
        try:
            ...
            logging.error("[ORDER_AGENT_V2] persist failed: %s", exc)
    ...
    def _dispatch_cta(
        ...
        *,
        ...
        try:
            ...
            logging.error("[ORDER_AGENT_V2] CTA dispatch raised: %s", exc)
            return False

    def _build_system_prompt(self, business_context: Optional[Dict]) -> str:
        from .order_agent import _format_business_info_for_prompt
...
def _read_cart_snapshot(wa_id: str, business_id: str) -> str:
    ...
    try:
        ...
        logging.warning(
            "[ORDER_AGENT_V2_DIAG] cart_read wa_id=%s items_count=%d "
        ...
        logging.warning("[ORDER_AGENT_V2] cart snapshot read failed: %s", exc)
        return ""
...
def _guard_impossible_envelope(
    ...
    if kind == "ready_to_confirm" and cart_was_empty_at_turn_start:
        ...
        logging.warning(
            "[ORDER_AGENT_V2] override_envelope kind=ready_to_confirm→chat "
    ...
    if kind == "order_placed":
        ...
        if "✅" not in place_order_output:
            logging.warning(
                "[ORDER_AGENT_V2] override_envelope kind=order_placed→%s "
