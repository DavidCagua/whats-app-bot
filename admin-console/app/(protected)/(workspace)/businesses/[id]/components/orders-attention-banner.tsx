...
const dismissKey = (businessId: string) =>
  `orders:attention-banner-dismissed:${businessId}`;
...
const dismissedSnapshotCache = new Map<
  string,
  { raw: string | null; parsed: Counts | null }
>();
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-3 gap-y-1">
        ...
        {counts.pending > 0 && counts.inFlight > 0 && (
          <span
            aria-hidden
            className="text-amber-900/40 dark:text-amber-100/40"
          >
            ·
          </span>
        )}
