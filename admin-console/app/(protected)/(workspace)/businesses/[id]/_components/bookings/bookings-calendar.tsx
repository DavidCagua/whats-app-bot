...
function BookingEventBlock({ event }: { event: CalendarEvent }) {
  ...
  return (
    <div
      className={`h-full px-1 py-0.5 rounded text-xs overflow-hidden ${textClass}`}
    >
      <p className="font-semibold truncate leading-tight">
        {booking.service?.name || "Reserva"}
      </p>
...
export function BookingsCalendar({
  ...
  const monthLabel = weekStart.toLocaleDateString("es-CO", {
    month: "long",
    year: "numeric",
  });
  const dayLabel = weekStart.toLocaleDateString("es-CO", {
    weekday: "short",
    month: "short",
    day: "numeric",
    year: "numeric",
