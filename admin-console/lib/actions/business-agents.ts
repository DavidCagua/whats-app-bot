...
export async function getBusinessAgents(businessId: string): Promise<BusinessAgentConfig[] | null> {
  try {
  ...
  try {
    ...
    for (const u of updates) {
      await prisma.business_agents.upsert({
        where: {
          business_id_agent_type: { business_id: businessId, agent_type: u.agent_type },
        },
        create: {
