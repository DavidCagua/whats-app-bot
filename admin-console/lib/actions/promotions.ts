...
function serialize(p: PromoRow): SerializedPromotion {
  return {
    ...
    fixed_price:
      p.fixed_price != null ? Number(p.fixed_price.toString()) : null,
    discount_amount:
...
export async function createPromotion(
  ...
  try {
    ...
    return {
      ...
      promotion: serialize(created as PromoRow),
    };
  } catch (err) {
...
export async function updatePromotion(
  ...
  if (!existing)
    return { success: false as const, error: "Promo no encontrada." };
  ...
  try {
    ...
    return {
      success: true as const,
      promotion: serialize(updated as PromoRow),
    };
  } catch (err) {
...
export async function deletePromotion(promotionId: string) {
  ...
  try {
    ...
    return {
      success: false as const,
      error:
        "No se puede borrar: la promo ya fue aplicada en pedidos. Desactívala mejor.",
    };
  }
}
